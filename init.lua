...
function Action.new(window, properties)
    ...
    self.window:Connect(self.interact.MouseButton1Click, function()
        ...
        task.spawn(function()
            ...
            if not success then
                log.warn(
                    "Library encountered an error, with the callback for a " .. tostring(self.__type) .. " component named '" .. tostring(self.name) .. "':"
                )
                log.print(result)
            end
            settleIcon()
...
function Button.new(tab, properties)
    ...
    if self.description and not self.compact then
        self.descriptor = __require("components.descriptor").new(self.tab, { description = self.description })
    end
...
function chrome.buildCollapsedFace(window)
    ...
    window.collapsedIcon = window:Create("ImageLabel", {
        ...
        Position = ((iconOnly) and UDim2.fromScale(0.5, 0.5) or UDim2.new(0), 16, 0.5, 0),
        Size = UDim2.fromOffset(24, 24),
        BackgroundTransparency = 1,
...
function chrome.setCollapsedShown(window, shown, tweenInfo)
    local targets = {
        [window.collapsedIcon] = { ImageTransparency = ((shown) and 0 or 1 }),
    }
...
function ColorPicker:_buildPicker()
    ...
    self.hexInput = self.window:Create("TextBox",, { TextColor3 = "ContentColor", FontFace = "Font", PlaceholderColor3 = "PlaceholderColor" })
...
function ColorPicker:_applyPickerVisibility(open, animate)
    local set = {
        [self.map] = { BackgroundTransparency = ((open) and 0 or 1 }),
        [self.satOverlay] = { BackgroundTransparency = ((open) and 0 or 1 }),
        [self.valOverlay] = { BackgroundTransparency = ((open) and 0 or 1 }),
        [self.mapStroke] = { Transparency = ((open) and 0.9 or 1 }),
        [self.satCursor] = { BackgroundTransparency = ((open) and 0 or 1 }),
        [self.satCursorStroke] = { Transparency = ((open) and 0 or 1 }),
        [self.hueBar] = { BackgroundTransparency = ((open) and 0 or 1 }),
        [self.hueHandle] = { BackgroundTransparency = ((open) and 0 or 1 }),
        [self.hueHandleStroke] = { Transparency = ((open) and 0 or 1 }),
        [self.alphaBar] = { BackgroundTransparency = ((open) and 0 or 1 }),
        [self.alphaHandle] = { BackgroundTransparency = ((open) and 0 or 1 }),
        [self.alphaHandleStroke] = { Transparency = ((open) and 0 or 1 }),
        [self.hexBox] = { BackgroundTransparency = ((open) and 0.9 or 1 }),
        [self.hexBoxStroke] = { Transparency = ((open) and 0.85 or 1 }),
        [self.hexInput] = { TextTransparency = ((open) and 0.4 or 1 }),
        [self.alphaBox] = { BackgroundTransparency = ((open) and 0.9 or 1 }),
        [self.alphaBoxStroke] = { Transparency = ((open) and 0.85 or 1 }),
        [self.alphaInput] = { TextTransparency = ((open) and 0.4 or 1 }),
    }
...
local function rowText(line): string
...
function Console:_setShown(shown, animate)
    ...
    if self.descriptor then
        w:_reveal(self.descriptor.titleLabel, { TextTransparency = ((shown) and 0.7 or 1 }), animate)
    end
end
...
__modules[[[components.descriptor]]] = function()
...
function Divider.new(tab, properties)
    ...
    local self = setmetatable({
        ...
        spacing = ((type(spacing) == "number") and math.max(spacing, 0) or defaultSpacing),
...
function Divider:_applyHeight()
    local content = ((self.text ~= "") and labelHeight elseif self.line then lineThickness or 0)
...
function Divider:_setShown(shown, animate)
    local w = self.window
    local ruleTransparency = ((shown) and lineShown or 1)
    ...
    w:_reveal(self.title, { TextTransparency = ((shown) and labelShown or 1 }), animate)
end
...
__modules[[[components.drag]]] = function()
...
function Dropdown.new(tab, properties)
    ...
    self.chevron = self.window:Create("ImageLabel",, { ImageColor3 = "ContentColor" })
    ...
    local function buildOption(optionName)
        ...
        local checkIcon = self.window:Create("ImageLabel",, { ImageColor3 = "ContentColor" })
...
function Dropdown:_buildSearch()
    ...
    self.searchIcon = self.window:Create("ImageButton",, { ImageColor3 = "ContentColor" })
...
function Dropdown:_syncScrollHint()
    ...
    variables.tweenService
        :Create(self.list, hintTween, { ScrollBarImageTransparency = ((more) and scrollbarShown or 1 }))
        :Play()
end
...
function Dropdown:_resizeToOptions()
    ...
    variables.tweenService:Create(self.main, searchTween,):Play()
end
...
local function listHeight(count): number
    return count * optionHeight + math.max(0, count - 1) * optionGap + listPadding * 2
end

local function rowsThatFit(space): number
...
function Dropdown:_setShown(shown, animate)
    ...
    if self.iconLabel then
        w:_reveal(self.iconLabel, { ImageTransparency = ((shown) and 0 or 1 }), animate)
    end
    if self.descriptor then
        w:_reveal(self.descriptor.titleLabel, { TextTransparency = ((shown) and 0.7 or 1 }), animate)
    end
...
function Group.new(tab, properties)
    ...
    self.main = self.window:Create("Frame", {
        ...
        BorderSizePixel = 0,
        AutomaticSize = Enum.AutomaticSize.Y,
        Size = ((horizontal) and UDim2.new(1, -20, 0, 0) or UDim2.new(1), 0, 0, 0),
...
function Group:_add(componentName, properties)
    if self.compact and not compactCapable[componentName] then
        log.warn(
            "Library: a row only holds compact elements (button/toggle/stat/slider), ignoring '" .. tostring(componentName) .. "'. Use a column for it."
        )
        return nil
    end
...
local constants = __require("utility.constants")
local hapticEngine = __require("utility.HapticEngine")
local enums = __require("utility.enums")
local log = __require("utility.log")
...
function Keybind:_beginHold(input)
    ...
    conn = self.window:ConnectFor(self, variables.userInputService.InputEnded, function(ended)
        local released = ((heldKeyCode ~= Enum.KeyCode.Unknown) and ended.KeyCode == heldKeyCode or ended.UserInputType == heldInputType)
...
__modules[[[components.popup]]] = function()
...
function Popup.new(window, properties)
    ...
    local self = setmetatable({
        ...
        dismissable = ((properties.dismissable ~= nil) and properties.dismissable
            elseif properties.Dismissable ~= nil then properties.Dismissable or true),
        _reveal = {},
        _connections = {},
        _closed = false,
    }, Popup)
...
function Popup:_buildContent()
    ...
    local window = self.window
    local measured = ((self.boxes and #self.boxes > 0) and self:_measureBoxes() or self:_measureText())
...
function Popup:_buildBox(parent, box, order)
    local window = self.window
    box = ((typeof(box) == "table") and box or { title = tostring(box)) }
...
function Popup:_measureHeader(hasIcon)
    ...
    self._subH = ((self.subtitle and self.subtitle ~= "") and functions.textHeight(self.window.theme.Font, subtitleSize, locale.resolve(self.subtitle), textWidth) or 0)
...
function Popup:_show()
    ...
    for _, entry in self._reveal do
        local info = ((entry.instance == self.backdrop) and backdropInfo or fadeLong)
...
__modules[[[components.progress]]] = function()
...
local function finite(value): number?
...
function Progress.new(tab, properties)
    ...
    steps = ((steps and steps >= 2) and math.floor(steps) or nil)
    ...
    local self = setmetatable({
        ...
        showValue = ((properties.showValue == nil) and true or properties.showValue == true),
...
function Progress:_clamp(value): number
...
function Progress:_build()
    ...
    self.readout = self.window:Create("TextLabel",, { TextColor3 = "ContentColor", FontFace = "Font" })
...
function Progress:_render(animate)
    ...
    if self.steps then
        ...
        for index, step in self.stepFrames do
            local target = ((index <= filled) and 0 or 1)
...
function Progress:_setShown(shown, animate)
    ...
    self._shown = shown
    w:_reveal(self.readout, { TextTransparency = ((shown) and readoutTransparency or 1 }), animate)
    ...
    if self.steps then
        local filled = self:_filledSteps()
        for index, step in self.stepFrames do
            w:_reveal(step.segment, { BackgroundTransparency = ((shown) and 0 or 1 }), animate)
            w:_reveal(step.fill, { BackgroundTransparency = ((shown and index <= filled) and 0 or 1 }), animate)
        end
        return
    end

    w:_reveal(self.track, { BackgroundTransparency = ((shown) and 0 or 1 }), animate)
    w:_reveal(self.fill, { BackgroundTransparency = ((shown) and 0 or 1 }), animate)
    w:_reveal(self.fillGlow, { Transparency = ((shown) and w.theme.AccentGlow or 1 }), animate)
end
...
__modules[[[components.search]]] = function()

local utility = __utility
local variables = __require("utility.variables")
local constants = __require("utility.constants")
local locale = __require("utility.locale")
local action = __require("components.action")
...
local function setPillShown(window, shown, info)
    variables.tweenService
        :Create(window.searchPill, info, {
            BackgroundTransparency = ((shown) and 0.9 or 1),
        })
        :Play()
    variables.tweenService:Create(window.searchStroke, info, { Transparency = ((shown) and 0.85 or 1 })):Play()
    variables.tweenService:Create(window.searchShadow, info, { Transparency = ((shown) and 0.92 or 1 })):Play()
    variables.tweenService:Create(window.searchIcon, info, { ImageTransparency = ((shown) and 0.65 or 1 })):Play()
    variables.tweenService:Create(window.searchInput, info, { TextTransparency = ((shown) and 0.2 or 1 })):Play()
end
...
function search.close(window, config)
    ...
    restoreUnits(window)
    local target = ((config.jumpTo == nil) and window.selectedTab and window.selectedTab.tabPage or config.jumpTo)
    if target then
...
function search.build(window)
    ...
    local top = ((sidebarLayout) and window.layout.chromeHeight + 8 or window.layout.tabStripTop + 1)
    ...
    window.searchIcon = window:Create("ImageLabel",, { ImageColor3 = "ContentColor" })
...
function Section.new(tab, properties)
    ...
    local topSpace = ((#self.tab.elements == 0) and 0 or 13)
...
function Section:_setShown(shown, animate)
    local w = self.window
    w:_reveal(self.title, { TextTransparency = ((shown) and 0.6 or 1 }), animate)
    if self.iconLabel then
        w:_reveal(self.iconLabel, { ImageTransparency = ((shown) and 0.65 or 1 }), animate)
    end
end
...
local image = __require("utility.image")
local locale = __require("utility.locale")
local tabSelector = __require("components.tabSelector")
local search = __require("components.search")
...
function sidebar.applyWidth(window, width)
    ...
    local collapsed = width < (layout.railWidth :)
    for _, tab in window.tabs do
    ...
    if window.profileContainer then
        ...
        window.profileLayout.HorizontalAlignment = ((collapsed) and Enum.HorizontalAlignment.Center or Enum.HorizontalAlignment.Left)
...
function sidebar.setProfileShown(window, shown, tweenInfo)
    ...
    local targets = {
        [window.profileAvatar] = {
            ImageTransparency = ((shown) and 0 or 1),
            BackgroundTransparency = ((shown) and avatarPlateTransparency or 1),
        },
        [window.profileName] = { TextTransparency = ((shown) and nameTransparency or 1 }),
        [window.profileSubtitle] = { TextTransparency = ((shown) and subtitleTransparency or 1 }),
    }
...
function sidebar.setSubtitle(window, text)
    ...
    local resolved = ((type(text) == "string" and text ~= "") and locale.resolve(text) or nil)
...
local variables = __require("utility.variables")
local functions = __require("utility.functions")
local odometer = __require("utility.odometer")
local moveable = __require("utility.moveable")
local lockable = __require("utility.lockable")
local locale = __require("utility.locale")
local hapticEngine = __require("utility.HapticEngine")
...
local function fillSize(ratio): UDim2
...
local function ratioFromPointer(x, left, travel): number
...
function Slider.new(tab, properties)
    ...
    self.value = (((properties.value or properties.Value) ~= nil) and (properties.value or properties.Value)
...
function Slider:_renderProgress(info)
    ...
    local ratio = ((span ~= 0) and math.clamp((self.value - self.range[1]) / span, 0, 1) or 0)
...
function Slider:_setHeld(held)
    ...
    local target = ((held) and handleHeld or handleRest)
    ...
    variables.tweenService
        ...
        :Play()
    variables.tweenService:Create(self.handleStroke, heldInfo, { Transparency = ((held) and 0.6 or 1 })):Play()
...
__modules[[[components.stat]]] = function()
...
function Statistic.new(tab, properties)
    ...
    local self = setmetatable({
        ...
        value = (((properties.value or properties.Value) ~= nil) and (properties.value or properties.Value) or 0),
        _hasValue = (properties.value or properties.Value) ~= nil,

        numberEasing = (((properties.numberEasing ~= nil)) and properties.numberEasing
            elseif properties.NumberEasing ~= nil then properties.NumberEasing or true),
        ...
        _initialValue = (((properties.value or properties.Value) ~= nil) and (properties.value or properties.Value) or nil),
        _lastChange = 0,
    }, Statistic)
    ...
    if self.description then
        if self.compact then
            log.warn("Library: a compact stat has no room for a description, ignoring it on '" .. tostring(self.name) .. "'.")
        else
            self.descriptor = __require("components.descriptor").new(self.tab, { description = self.description })
        end
    end
...
function Statistic:_buildCompact()
    ...
    self.main = window:Create("Frame", {
        Name = self.name,
        Size = ((inRow) and UDim2.fromOffset(0, compactHeight) or UDim2.new(1), -20, 0, compactHeight),
        AutomaticSize = ((inRow) and Enum.AutomaticSize.X or Enum.AutomaticSize.None),
        ClipsDescendants = true,
        BorderSizePixel = 0,
        ZIndex = 5,
...
function Statistic:_setAccent(key, direction)
    ...
    local rotation = ((direction == 1) and 14 elseif direction == -1 then -10 or 2)
    local offset = ((direction == 1) and Vector2.new(0.04, 0.5)
        elseif direction == -1 then Vector2.new(-0.04, 0.5) or Vector2.new(0), 0.5)
...
function Statistic:Set(value)
    ...
    local refValue = ((self.changeBaseline == "initial") and self._initialValue or previousValue)
    ...
    elseif refValue == 0 then
        ...
        local suffix = ((self.changeMode == "percentage") and "∞%" or "∞")
        infinityText = sign .. suffix
        accentKey = ((value > 0) and "positive" or "negative")
        direction = ((value > 0) and 1 or -1)
    elseif self.changeMode == "percentage" then
        targetChange = ((value - refValue) / math.abs(refValue)) * 100
        direction = ((targetChange > 0) and 1 elseif targetChange < 0 then -1 or 0)
        accentKey = ((direction == 1) and "positive" elseif direction == -1 then "negative" or "neutral")
    else
...
function Statistic:ResetBaseline(newBaseline)
    local previousValue = self.value
    local hasNoHistory = not self._hasValue
    local baseline = ((typeof(newBaseline) == "number") and newBaseline or self.value)
...
function Statistic:_setShown(shown, animate)
    ...
    if self.compact then
        w:_reveal(
            self.main,
            { BackgroundTransparency = ((shown) and (self.window.theme.ElementTransparency or 0) or 1 }),
            animate
        )
        w:_reveal(
            self.stroke,
            { Transparency = ((shown) and self.window.theme.ElementStrokeTransparency or 1 }),
            animate
        )
        w:_reveal(self.card, { BackgroundTransparency = ((shown) and 0 or 1 }), animate)
        w:_reveal(self.title, { TextTransparency = ((shown) and 0 or 1 }), animate)
        if self.iconLabel then
            w:_reveal(self.iconLabel, { ImageTransparency = ((shown) and 0 or 1 }), animate)
        end
...
function Statistic:_minWidth()
    ...
    w += functions.textWidth(self.window.theme.Font, 16, locale.resolve(self.name))
    local readout = ((self.display == "change") and self:_formatChange(0) or self:_formatValue(self.value))
...
function Tab:CreateButton(properties)
    return self:_register(__require("components.button").new(self, properties))
end

function Tab:CreateToggle(properties)
    return self:_register(__require("components.toggle").new(self, properties))
end
...
function Tab:CreateSection(properties)
    return self:_register(__require("components.section").new(self, properties))
end

function Tab:CreateText(properties)
    return self:_register(__require("components.text").new(self, properties))
end

function Tab:CreateDivider(properties)
    return self:_register(__require("components.divider").new(self, properties))
end

function Tab:CreateProgress(properties)
    return self:_register(__require("components.progress").new(self, properties))
end

function Tab:CreateConsole(properties)
    return self:_register(__require("components.console").new(self, properties))
end

function Tab:CreateStat(properties)
    return self:_register(__require("components.stat").new(self, properties))
end

function Tab:CreateSlider(properties)
    return self:_register(__require("components.slider").new(self, properties))
end

function Tab:CreateDropdown(properties)
    return self:_register(__require("components.dropdown").new(self, properties))
end

function Tab:CreateInput(properties)
    return self:_register(__require("components.input").new(self, properties))
end

function Tab:CreateKeybind(properties)
    return self:_register(__require("components.keybind").new(self, properties))
end

function Tab:CreateColorPicker(properties)
    return self:_register(__require("components.colorpicker").new(self, properties))
end

function Tab:CreateGroup(properties)
    return self:_register(__require("components.group").new(self, properties))
end
...
__modules[[[components.tabSection]]] = function()
...
__modules[[[components.tabSelector]]] = function()
...
local function initialOf(name): string
    if type(name) ~= "string" or name == "" then
        return "?"
    end
    local afterFirst = utf8.offset(name, 2)
    local first = ((afterFirst) and string.sub(name, 1, afterFirst - 1) or name)
...
function tabSelector.setRowCollapsed(tab, collapsed, layout)
    ...
    local inset = ((collapsed) and 0 or layout.rowPadding)
    tab.topbarItemContainer.Size = UDim2.new(1, -inset, 0, 24)
    tab.topbarItemContainer.Position = UDim2.new(0, inset, 0.5, 0)
    tab.topbarItemLayout.HorizontalAlignment = ((collapsed) and Enum.HorizontalAlignment.Center or Enum.HorizontalAlignment.Left)
end
...
function Tag:_setShown(shown, animate)
    local target = ((shown) and 0 or 1)
    local info = ((typeof(animate) == "TweenInfo") and animate elseif animate then setTweenInfo or nil)
    if info then
...
__modules[[[components.text]]] = function()
...
function Text.new(tab, properties)
    ...
    self.main = self.window:Create("Frame", {
        ...
        AutomaticSize = Enum.AutomaticSize.Y,
        BorderSizePixel = 0,
        Name = ((self.name ~= "") and self.name or "Text"),
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
...
function Text:_setShown(shown, animate)
    ...
    self.window:_reveal(self.body, { TextTransparency = ((shown) and bodyShown or 1 }), animate)
end
...
__modules[[[components.toast]]] = function()
...
function Toast.new(window, properties, container)
    ...
    self._iconImage = ((hasAvatar) and image.avatar(self.avatar, function(uri)
            if self.iconLabel and not self._dismissed and self.main.Parent then
                image.assign(self.iconLabel, "Image", uri)
            end
        end)
        elseif hasIcon then resolveImage(self.icon) or nil)
    self._iconSize = ((hasAvatar) and avatarSize or iconSize)
    self._leftPad = ((hasAvatar) and avatarLeftPadding or leftPadding)
    self._rightPad = ((hasAvatar) and avatarRightPadding or rightPadding)
    ...
    self.body = self.window:Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        Size = UDim2.new(1, 0, 1, 0),
        Position = ((self.position == "Bottom") and offscreenBelow or offscreenAbove),
        AnchorPoint = Vector2.new(0.5, 0.5),
        Active = true,
        BorderSizePixel = 0,
        ZIndex = constants.zIndex.toast,
    ...
    self.titleLabel = self.window:Create("TextLabel", {
        ...
        TextSize = 16,
        TextXAlignment = Enum.TextXAlignment.Left,
        LayoutOrder = ((self.subtitleAbove) and 2 or 1),
        ZIndex = constants.zIndex.toastContent,
    ...
    if hasSubtitle then
        self.subtitleLabel = self.window:Create("TextLabel", {
            ...
            TextSize = 14,
            TextXAlignment = Enum.TextXAlignment.Left,
            LayoutOrder = ((self.subtitleAbove) and 1 or 2),
            ZIndex = constants.zIndex.toastContent,
...
function Toast:_measure()
    ...
    local subtitleWidth = ((self.subtitleLabel) and functions.textWidth(self.window.theme.Font, 14, self.subtitle) or 0)
    local textWidth = math.max(titleWidth, subtitleWidth)

    local left = ((self.iconLabel) and self._leftPad + self._iconSize + iconGap or self._leftPad)
    ...
    local textHeight = ((self.subtitleLabel) and 16 + 1 + 14 or 16)
...
function Toggle.new(tab, properties)
    ...
    local self = setmetatable({
        ...
        value = (((properties.value or properties.Value) ~= nil) and (properties.value or properties.Value) or false),
    }, Toggle)
...
local chrome = __require("components.chrome")
local search = __require("components.search")
local sidebar = __require("components.sidebar")

local variables = __require("utility.variables")

local themes = __themes
...
local function setTopToastPosition(window, animate)
    ...
    local target = ((window._collapsedShown) and topToastClosedPosition or topToastOpenPosition)
...
local function edgeShade(color, amount)
    ...
    local target = ((luminance > 0.5) and Color3.new(0, 0, 0) or Color3.new(1), 1, 1)
...
local function themeOverrides(value)
    ...
    return __require("themes.default")
end

local function resolveTheme(value)
    local resolved = table.clone(__require("themes.default"))
    ...
    local userTable = ((typeof(value) == "table") and value or nil)
...
function Window.new(properties)
    ...
    self.drag = __require("components.drag").new(self)
    ...
    __require("components.action").new(self,)

    self.minimiseAction = __require("components.action").new(self,)

    self.settingsAction = __require("components.action").new(self,)
...
function Window:ChangeTheme(theme)
    local overrides = ((typeof(theme) == "table") and theme or resolveTheme(theme))
    ...
    for instance, properties in self.themeProperties do
        for property, value in properties do
            local targetValue = ((typeof(value) == "table") and value[2](self.theme[value[1]]) or self.theme[value)]
...
function Window:CreateSection(properties)
    ...
    local section = __require("components.tabSection").new(self, properties)
...
function Window:CreateTag(properties)
    ...
    local newTag = __require("components.tag").new(self, properties)
...
function Window:Notify(properties)
    ...
    return __require("components.notification").new(self, properties)
end
...
function Window:Toast(properties)
    ...
    properties.position = ((isTop) and "Top" or "Bottom")
    local containerKey = ((isTop) and "_toastsTop" or "_toastsBottom")
    ...
    if not container then
        container = self:Create("Frame", {
            ...
            Position = ((isTop) and topToastOpenPosition or UDim2.new(0.5), 0, 1, -12),
            AnchorPoint = ((isTop) and Vector2.new(0.5, 0) or Vector2.new(0.5), 1),
            BackgroundTransparency = 1,
            ZIndex = constants.zIndex.toast,
    ...
    return __require("components.toast").new(self, properties, container)
end
...
function Window:Popup(properties)
    ...
    return __require("components.popup").new(self, properties)
end
...
function Window:_clampedPosition(position): UDim2
...
function Window:_watchViewport()
    ...
    self:Connect(variables.runService.Heartbeat, function(delta)
...
function Window:_setElementLocked(element, locked, reason)
    ...
    variables.tweenService
        :Create(element.lockScrim, info, {
            BackgroundTransparency = ((locked) and lockScrimTransparency or 1),
        })
        :Play()
    if not locked then
    ...
    variables.tweenService
        :Create(descriptor.titleLabel, info, {
            TextTransparency = ((locked) and lockedDescriptionTransparency or 0.7),
        })
        :Play()
end
...
function Window:_fadeSurfaces(shown, fadeInfo)
    local targets = {
        [self.windowShadow] = { Transparency = ((shown) and 0.6 or 1 }),
        [self.windowStroke] = { Transparency = ((shown) and 0.95 or 1 }),
        [self.bottomFade] = { BackgroundTransparency = ((shown) and 0 or 1 }),
    }

    if self.elementsStroke then
        targets[self.elements] = {
            BackgroundTransparency = ((shown) and self.layout.cardTransparency or 1),
        }
        targets[self.elementsStroke] = { Transparency = ((shown) and 0 or 1 })
    end
...
function Window:_runGuarded(element, fn, ...)
    ...
    task.spawn(function()
        ...
        log.warn(
            "Library encountered an error, with the callback for a " .. tostring(element.__type) .. " component named '" .. tostring(element.name) .. "':"
        )
        log.print(err)
...
function Window:_collapsedRect()
    local size = ((self.showIconOnly) and collapsedIconSize or collapsedSize)
...
__modules[[[themes.cobalt]]] = function()
...
__modules[[[themes.ember]]] = function()
...
__modules[[[themes.frost]]] = function()
...
__modules[[[utility.HapticEngine]]] = function()

local variables = __require("components.variables")
...
function hapticEngine.setContainer(target)
    container = target
end

function hapticEngine.releaseContainer(target)
...
__modules[[[utility.assetResolver]]] = function()

local network = __require("components.network")
local log = __require("components.log")
...
assetResolver.__type = "assetResolver"
    ...
    resolve: (self, value) -> ResolvedAsset?,
    getAssetContentFromUrl: (self, url, cacheKey, forced) -> string?,
    getAssetContentFromId: (self, id, forced) -> string?,
}
...
function assetResolver.new(useCache, assetContentDownloadUrl): AssetResolver
    local contentCache:? = ((useCache) and {} or nil)
    local self = setmetatable(
        {
            contentCache = contentCache,
            contentCacheOrder = ((useCache) and {} } or nil),
...
function assetResolver.resolve(_self, value): ResolvedAsset?
...
local function isGoodResponse(response): boolean
    if type(response) ~= "table" then
        return false
    end
    local shaped = response }
    local body = shaped.Body
    if type(body) ~= "string" or #body == 0 then
        return false
    end
    if type(shaped.StatusCode) == "number" then
        local statusCode = shaped.StatusCode
        ...
        if success and isGoodResponse(response) then
            ...
            if cacheKey ~= nil and contentCache then
                ...
                if order and contentCache[cacheKey] == nil then
                    ...
                    local oldest = ((#order > contentCacheLimit) and table.remove(order, 1) or nil)
                    if oldest ~= nil then
...
function assetResolver.getAssetContentFromId(self, id, forced): string?
...
__modules[[[utility.colors]]] = function()

local colors = {}

function colors.contrastColor(color): Color3
    local luminance = 0.299 * color.R + 0.587 * color.G + 0.114 * color.B
...
function colors.contrastText(color): Color3
    local luminance = 0.299 * color.R + 0.587 * color.G + 0.114 * color.B
...
__modules[[[utility.constants]]] = function()
...
__modules[[[utility.enums]]] = function()
...
__modules[[[utility.filesystem]]] = function()

local services = __require("components.services")
...
if hasNativeFS then
    ...
    function filesystem.isfile(path): boolean
        return isfile(path)
    end
    ...
    function filesystem.listfiles(folder): { string }
    ...
    function filesystem.isfolder(path): boolean
...
elseif isStudio then
    ...
    local function resolveParent(parts: { string }, createMissing): Instance?
        local current = root
        for i = 1, #parts - 1 do
    ...
    local function fileFromInstance(instance): StringValue?
...
if filesystem.writefile and not filesystem.appendfile then
    function filesystem.appendfile(path, content)
        local existing = ((filesystem.isfile(path)) and filesystem.readfile(path) or nil)
...
__modules[[[utility.filesystemManager]]] = function()

local filesystem = __require("components.filesystem")
local path = __require("components.path")
...
fileSystemManager.__index = fileSystemManager
    ...
    root,
    assets,
    getPath: (self, subpath) -> string,
    getAssetsFolder: (self, subfolder) -> string,
    getRootFolder: (self) -> string,
}

function fileSystemManager.new(name): FileSystemManager
...
function fileSystemManager:getPath(subpath): string
    return path.join(self.root, subpath)
end

function fileSystemManager:getAssetsFolder(subfolder): string
    if subfolder then
...
__modules[[[utility.flagNames]]] = function()
...
local function hashName(name): number
...
function flagNames.deriveFlagFromName(name): string
    local flag = name:gsub("(%S+)", function(w): string
...
__modules[[[utility.fontManager]]] = function()

local services = __require("components.services")
local httpService = services.getService("HttpService")
local runService = services.getService("RunService")
local fileSystem = __require("components.filesystem")
local assetResolver = __require("components.assetResolver")
local log = __require("components.log")
local path = __require("components.path")

local function jsonEncode(data): string?
...
local function jsonDecode(jsonString): unknown?
    local success, result = pcall(function()
...
fontManager.__type = "fontManager"
    ...
    resolve: (self, id: number | string) -> Font?,
    loadFont: (
        self,
        id: (number | string)?,
    ...
    getFontFromId: (self, id: number | string) -> Font?,
}

local function validateManifest(manifest): FontManifest?
...
local function isFontBody(body): boolean
...
local function resolveId(id): number?
...
function fontManager.__index(self, key): unknown
...
local function fetchFont(
    ...
    local possibleFontManifest = nil
    ...
    for i, face in ipairs(fontManifest.faces) do
        ...
        if not fileSystem.isfile(fontFacePath) then
            ...
            local fontFaceContent = ((fontFaceId ~= nil) and self.assetResolver:getAssetContentFromId(fontFaceId, false) or nil)
    ...
    local ok, loadedFont = pcall(Font.new, fontManifestId :, requestedFontWeight, requestedFontStyle)
...
function fontManager.loadFont(
    ...
    if fontCache and not skipCache then
        ...
        if cachedFont then
            ...
            local ok, loadedFont = pcall(Font.new, cachedFont.customId :, requestedFontWeight, requestedFontStyle)
    ...
    local loadedFont = ((ok) and result or self.defaultOptions.fallbackFont)
...
function fontManager.resolve(self, id: number | string): Font?
...
function fontManager.getFontFromId(self, id: number | string): Font?
...
__modules[[[utility.functions]]] = function()

local functions = {}

local textMetrics = __require("components.textMetrics")
local colors = __require("components.colors")
local flags = __require("components.flagNames")
...
__modules[[[utility.image]]] = function()

local imageCache = __require("components.imageCache")
local variables = __require("components.variables")
...
local function idOf(value): number?
    if type(value) == "number" then
        return value
...
function image.preload(onSettled): (boolean, number)
    settled = false
...
function image.avatar(userId, onReady): string
...
function image.resolve(value): string
    ...
    local rewrite = ((id) and image.rewrites[id] or nil)
    if rewrite then
        return rewrite
    end
...
__modules[[[utility.imageCache]]] = function()

local filesystem = __require("components.filesystem")
local path = __require("components.path")
local variables = __require("components.variables")
local constants = __require("components.constants")
...
local function cacheFile(filePath, url): string?
...
local function avatarPath(userId): string
    return path.join(cacheFolder, "avatar_" .. tostring(userId) .. ".png")
end

local function decodeThumbnailUrl(body): string?
...
local function fetchAvatar(userId): string?
    local cdnUrl = nil
    for attempt = 1, 4 do
...
function imageCache.preload(onSettled): (boolean, number)
    local env = getfenv()
    ...
    for id, url in manifest do
        ...
        if filesystem.isfile(filePath) then
            ...
            uri = ((ok and type(res) == "string") and res or nil)
        end
        if uri then
            rewrites[id] = uri
        else
...
function imageCache.avatar(userId, onReady): string
...
__modules[[[utility.layouts]]] = function()
...
function layouts.railWidthFor(layout, windowWidth)
    ...
    if windowWidth < (layout.railCollapseBelow :) then
        return layout.railCollapsedWidth
    end
    return full
end
...
__modules[[[utility.locale]]] = function()
...
local function languageOf(id): string
    return string.match(id, "^(%a+)") or id
end

function locale.t(source): unknown
...
function locale.isToken(value): boolean
    return type(value) == "table" and type((value })[tokenTag]) == "string"
end

function locale.sourceOf(token): string
    return token[tokenTag]
end

function locale.resolve(source): any
        ...
        if type(id) == "string" and type(entries) == "table" then
            ...
            for source, translated in entries do
                if type(source) == "string" and type(translated) == "string" then
                    ...
                    log.warn("Library: skipping a '" .. tostring(id) .. "' translation, entries must be string to string.")
                end
            end
        end
    end
...
function locale.setActive(localeId): string
    locale.current = ((type(localeId) == "string" and localeId ~= "") and string.lower(localeId) or "en")
...
__modules[[[utility.lockable]]] = function()

local function lockable<T>(class): T
    local target = class

    function target:Lock(reason)
...
local runtime = __require("components.runtime")
...
local secureModeSource = defaultSecureModeSource
local suppressPredicate = nil

function log.setSecureModeSource(source)
    secureModeSource = ((type(source) == "function") and source or defaultSecureModeSource)
end

function log.setSuppressPredicate(predicate)
    suppressPredicate = ((type(predicate) == "function") and predicate or nil)
end
...
__modules[[[utility.moveable]]] = function()

local function moveable<T>(class): T
    local target = class

    function target:MoveTo(index)
        self.tab:_moveElement(self, index)
    end
...
__modules[[[utility.network]]] = function()

local network = {}
network.__index = network

function network.getRequestFn(env): RequestFn?
    env = env or getfenv()
    return env.request
        or env.http_request
...
__modules[[[utility.odometer]]] = function()
...
__modules[[[utility.ordering]]] = function()
...
__modules[[[utility.path]]] = function()

local path = {}

function path.join(basePath, childPath): string
...
local function stripTraversal(text): string
    local previous
    repeat
        previous = text
        text = text:gsub("%.%.", "")
...
function path.sanitizeFolder(value): string
    local text = tostring(value):gsub("\\", "/"):gsub('[:<>"|?*%c]', "")
    text = stripTraversal(text):gsub("/+", "/")
    return (text:gsub("^/+", ""))
end

function path.sanitizeFile(value): string
    local text = tostring(value):gsub("[/\\]", ""):gsub('[:<>"|?*%c]', "")
    return stripTraversal(text)
end

function path.basename(value): string
    return tostring(value):match("[^/\\]+$") or tostring(value)
end

function path.stripExtension(value, extension): string
...
__modules[[[utility.persistence]]] = function()

local persistence = {}

local config = __require("components.persistenceConfig")
local settings = __require("components.persistenceSettings")
...
__modules[[[utility.persistenceConfig]]] = function()

local variables = __require("components.variables")
local filesystem = __require("components.filesystem")
local log = __require("components.log")
local path = __require("components.path")
local paths = __require("components.persistencePaths")
local atomic = __require("components.persistenceWrite")
...
function persistenceConfig.getPath(window, name): (string?, string?)
    return paths.getConfigPath(window, name)
end

function persistenceConfig.save(window, name): boolean
...
local function decodeFile(fullPath): (?, string?)
...
local function backupPathFor(fullPath): string
...
function persistenceConfig.load(window, name): boolean
    local dir, fullPath = persistenceConfig.getPath(window, name)
    if not dir or not fullPath then
        log.warn("Library: configuration name '" .. tostring(name) .. "' has no usable characters")
        return false
...
function persistenceConfig.applyTo(control, value)
...
function persistenceConfig.list(window): { string }
...
function persistenceConfig.delete(window, name): boolean
    if type(name) ~= "string" or name == "" then
        return false
    end
...
__modules[[[utility.persistencePaths]]] = function()
...
function paths.getConfigPath(window, name): (string?, string?)
...
__modules[[[utility.persistenceSettings]]] = function()

local variables = __require("components.variables")
local filesystem = __require("components.filesystem")
local paths = __require("components.persistencePaths")
local atomic = __require("components.persistenceWrite")
local enums = __require("components.enums")
...
function persistenceSettings.saveSettings(window): boolean
...
local function decodeFile(fullPath): (DecodedSettings?, string?)
...
function persistenceSettings.loadSettings(window): boolean
    local dir, fullPath = persistenceSettings.getSettingsPath()
...
__modules[[[utility.persistenceWrite]]] = function()
...
function persistenceWrite.tempPathFor(fullPath): string
    return fullPath .. parkedExtension
end

function persistenceWrite.write(dir, fullPath, contents)
...
__modules[[[utility.runtime]]] = function()
...
runtime.workspace = services.getService("Workspace")
...
__modules[[[utility.services]]] = function()
...
__modules[[[utility.textMetrics]]] = function()
...
local function getTextBounds(params): unknown
...
local function boundAxis(bounds, axis: "X" | "Y"): number?
    if typeof(bounds) == "Vector2" then
        return if axis == "X" then bounds.X else bounds.Y
    end
    if type(bounds) == "table" and type((bounds :)[axis]) == "number" then
        return (bounds :)[axis]
    end
    return nil
end

function textMetrics.textWidth(font, size, text): number
    text = tostring(text)
    local key = tostring(font.Family)
        .. "|"
        .. tostring(font.Weight)
...
function textMetrics.textHeight(font, size, text, width): number
    local params = Instance.new("GetTextBoundsParams")
...
local constants = __require("components.constants")
local log = __require("components.log")
local fileSystemManager = __require("components.filesystemManager")
local assetResolver = __require("components.assetResolver")
local fontManager = __require("components.fontManager")
...
variables.assetResolver =
    assetResolver.new(true, assetResolver.Enum.AssetDownloadUrl.RoProxyDownloadUrl)
...
__modules[[[utility.windowSizing]]] = function()

local layouts = __require("components.layouts")
...
function windowSizing.pageHeight(windowHeight, mode: layouts.Mode?): number
    local profile = windowSizing.profile(mode)
    local height = ((windowHeight and windowHeight > 0) and windowHeight or profile.minSize.Y)
    return math.max(height - profile.chromeHeight, 0)
end

local function availableHeight(profile, viewportY): number
...
local function deficit(actual, ideal, floor): number
    return math.clamp((ideal - actual) / (ideal - floor), 0, 1)
end

local function fitVertical(profile, availableX, availableY): UDim2
    ...
    local width = math.max(profile.defaultSize.X + (profile.widthCompensation :) * lost, profile.minSize.X)

    return UDim2.fromOffset(math.floor(math.min(width, availableX, height * profile.maxAspectRatio)), height)
end

local function fitHorizontal(profile, availableX, availableY): UDim2
    ...
    height = math.floor(math.min(height, width / (profile.minAspectRatio :)))
    width = math.floor(math.min(width, height * profile.maxAspectRatio))

    return UDim2.fromOffset(width, height)
end

function windowSizing.fit(viewport, mode: layouts.Mode?): UDim2
    local profile = windowSizing.profile(mode)
...
function Library:CreateWindow(properties: types.WindowProps): types.Window
        ...
        image.preload(function(failed)
            ...
            local function notify()
                ...
                window:Notify({
                    ...
                    content = ((failed == 1) and locale.resolve("An asset couldn't be cached and won't appear.") or locale.resolve("Some assets couldn't be cached and won't appear.")),
                })
            end
            if window then
                notify()
    ...
    local made, result = pcall(function()
        return (__require("components.window") :).new(properties)
    end)
    if not made then
        banner:Destroy()
        error(result, 0)
...
Library.Theme = {
    GetThemeNames = function()
        return { "Default", "Amethyst", "Cobalt", "Ember", "Frost", "Rose" }
    end,
}

local function normalizeWindowProperties(properties)
    if typeof(properties) ~= "table" then
        return { name = properties }
    end
    local out = table.clone(properties)
    if out.name == nil and out.Name == nil and out.Title ~= nil then
        out.name = out.Title
    end
    return out
end

local function normalizeTabProperties(properties)
    if typeof(properties) == "string" then
        return { name = properties }
    end
    if typeof(properties) ~= "table" then
        return properties
    ...
    local out = table.clone(properties)
    if out.name == nil and out.Name == nil and out.Title ~= nil then out.name = out.Title end
    return out
end

local function normalizeControlProperties(properties, kind)
    if typeof(properties) ~= "table" then return properties end
    local out = table.clone(properties)
    if out.name == nil and out.Name == nil and out.Title ~= nil then out.name = out.Title end
    if kind == "toggle" and out.value == nil and out.Value == nil and out.Default ~= nil then out.value = out.Default end
    if kind == "slider" then
        if out.range == nil and out.Range == nil and (out.Min ~= nil or out.Max ~= nil) then
            out.range = { out.Min or 0, out.Max or 100 }
        end
        if out.value == nil and out.Value == nil and out.Default ~= nil then out.value = out.Default end
    elseif kind == "dropdown" then
        if out.options == nil and out.Options ~= nil then out.options = out.Options end
        if out.value == nil and out.Value == nil and out.Default ~= nil then out.value = out.Default end
    end
    return out
end
...
Library.CreateWindow = function(self, properties)
    properties = normalizeWindowProperties(properties)
    local built = rawCreateWindow(self, properties)
    local rawCreateTab = built.CreateTab
    local rawChangeTheme = built.ChangeTheme
    built.SetTheme = function(window, theme)
        return rawChangeTheme(window, theme)
    end
    built.CreateTab = function(window, tabProperties)
        local tab = rawCreateTab(window, normalizeTabProperties(tabProperties))
        local methods = {
            CreateButton = "button", CreateToggle = "toggle", CreateSlider = "slider", CreateDropdown = "dropdown",
        }
        for method, kind in pairs(methods) do
            local raw = tab[method]
            tab[method] = function(t, props)
                return raw(t, normalizeControlProperties(props, kind))
            end
        end
        return tab
    end
...
local function __require(name)
    ...
    if __loading[name] then error("Circular module dependency: " .. tostring(name), 2) end
    local loader = __modules[name]
    if not loader then error("Library module not found: " .. tostring(name), 2) end
    __loading[name] = true
